...
export async function clientLoader({ params, request }) {
  ...
  const response = await fetch(`${url.origin}/content/${projectMeta.slug}/gallery.json?t=${Date.now()}`);
  if (!response.ok) {
...
export default function ProjectDetail() {
  ...
  const allImages = projectData.media || [];
  ...
  return (
    <div className="w-full min-h-screen bg-primary">
      <SEO 
        title={`Havilah | ${projectMeta.title}`}
        description={projectMeta.story}
        path={`/projects/${projectMeta.id}`}
        image={projectMeta.cover || projectMeta.heroImage}
      />
          ...
          <h1 className="text-6xl md:text-[8vw] font-display uppercase tracking-tighter text-white/80 opacity-70 mb-2 md:mb-4 flex flex-wrap justify-center gap-x-[2vw]">
            {projectMeta.title.split(' ').map((word, i) => (
              <span key={i} className="overflow-hidden inline-block pb-4 md:pb-8 -mb-4 md:-mb-8">
                ...
                >
                  <GradientShimmer gradient="tonic" duration={2}>{word}</GradientShimmer>
                </motion.span>
              </span>
            ))}
          </h1>
      ...
      <section className="py-24 px-4 md:px-12 max-w-7xl mx-auto w-full flex flex-col xl:flex-row gap-12">
        ...
        <div className="w-full xl:w-1/3 flex flex-col gap-8">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-[10px] font-mono tracking-widest text-white/40 uppercase mb-1">Client</p>
              <p className="text-sm uppercase tracking-wider">{projectData.credits?.client || 'Internal'}</p>
            </div>
            <div>
              <p className="text-[10px] font-mono tracking-widest text-white/40 uppercase mb-1">Category</p>
              <p className="text-sm uppercase tracking-wider">{projectMeta.category}</p>
            </div>
          </div>
          <div className="pt-6 border-t border-white/10">
            <p className="text-[10px] font-mono tracking-widest text-white/40 uppercase mb-3">Deliverables</p>
            <ul className="flex flex-wrap gap-2">
              {projectData.deliverables?.map((item, i) => (
                <li key={i} className="text-xs uppercase tracking-widest border border-white/20 px-3 py-1 rounded-full text-white/80">
                  {item}
                </li>
              ))}
            </ul>

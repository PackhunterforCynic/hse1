...
import { shouldSkipEntranceAnimation, recordAnimationComplete } from '../lib/cacheEngine';
...
export default function Preloader() {
  ...
  useLayoutEffect(() => {
    if (phase === 'flying' && logoRef.current) {
      ...
      if (navLogoEl && source.width > 0) {
        ...
        setFlightTransform({
          x: isMobile ? "-30vw" : "-38vw",
          y: isMobile ? "-42vh" : "-44vh",
  ...
  useEffect(() => {
    ...
    // Check smart cache flag if intro has already played in session or within reload window

  ...
  return (
    ...
    >
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={cn(
                  "absolute left-1/2 top-0 aspect-square overflow-hidden rounded-2xl bg-[#111] border border-white/15 shadow-[0_20px_50px_rgba(0,0,0,0.8)] will-change-transform",
                  cardClassName,
                )}
        ...
        {showNavigation && (
          <>
            <button
              ...
              onClick={() => nudge(-1)}
              className="absolute left-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-black/70 border border-white/20 p-2 text-white backdrop-blur-md transition hover:bg-white/20 hover:scale-105"
            >
            ...
            <button
              ...
              onClick={() => nudge(1)}
              className="absolute right-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-black/70 border border-white/20 p-2 text-white backdrop-blur-md transition hover:bg-white/20 hover:scale-105"
            >
      ...
      {showCaption && active?.title && (
        ...
        >
          <p className="text-xl md:text-2xl font-display uppercase tracking-tight text-white">
            <GradientShimmer gradient="tonic" duration={1.6}>{active.title}</GradientShimmer>
          </p>
          {active.subtitle && (
            <p className="mt-1 text-sm text-accent font-mono tracking-widest uppercase">
              {active.subtitle}
            </p>
          )}
          {active.meta && active.meta.length > 0 && (
            <dl className="mt-6 w-full max-w-sm text-xs font-mono border border-white/10 rounded-xl p-4 bg-white/5 backdrop-blur-sm">
              {active.meta.map((row) => (
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          {slides.map((_, index) => (
            <button
              ...
              onClick={() => goTo(index)}
              className={cn(
                "size-2 rounded-full bg-accent transition-all duration-300",
                index === selected ? "opacity-100 scale-125 w-6" : "opacity-30 hover:opacity-70",
              )}
            />
          ))}
        </div>
